...
```bash
# Backup MongoDB
mongodump --uri="mongodb://localhost:27017/lenquant" --out=backup_before_phase2

# Verify backup
ls -lh backup_before_phase2/
```

### Step 2: Get Your User ID

First, login to the system and get your user ID:
...
with mongo_client() as client:
    ...
    user = db.users.find_one({"email": "your-email@gmail.com"})
    if user:
...
Save your `user_id` for the migration (e.g., `google_123456789`).

### Step 3: Create Migration Script

Create `db/migrations/002_add_user_id_to_collections.py`:

```python
"""Add user_id field to user-specific collections."""
...
DEFAULT_USER_ID = os.getenv("MIGRATION_USER_ID", "google_YOUR_ID_HERE")


def migrate_collection(db, collection_name: str, user_id: str) -> int:
    """Add user_id to all documents in a collection."""
    ...
    # Add user_id to all documents without it
    result = collection.update_many(
        {"user_id": {"$exists": False}},
        {"$set": {"user_id": user_id, "migrated_at": datetime.utcnow()}}
    )
...
def add_indexes(db):
    """Add user_id indexes to collections."""
    ...
    for collection_name in collections_to_index:
        ...
        elif collection_name == "models.registry":
            collection.create_index([("user_id", 1), ("symbol", 1), ("horizon", 1)])
        elif collection_name == "positions":
            collection.create_index([("user_id", 1), ("symbol", 1)])
        elif collection_name == "orders":
            collection.create_index([("user_id", 1), ("symbol", 1), ("timestamp", -1)])
        else:
            collection.create_index([("user_id", 1)])
        
        print(f"  ✅ Created index on {collection_name}.user_id")


def migrate():
    """Run Phase 2 migration."""
    print("=" * 60)
    print("Phase 2 Migration: Adding user_id to collections")
    print("=" * 60)
    
    if DEFAULT_USER_ID == "google_YOUR_ID_HERE":
        print("\n❌ ERROR: You must set your user_id first!")
        print("\nSteps:")
        print("1. Login to the system via web interface")
        print("2. Run: python -c \"from db.client import mongo_client, get_database_name; import sys; client = mongo_client.__enter__(); db = client[get_database_name()]; user = db.users.find_one({'email': 'YOUR_EMAIL'}); print(f'User ID: {user[\"id\"]}') if user else print('Not found')\"")
        print("3. Set MIGRATION_USER_ID environment variable")
        print("4. Re-run this script")
        sys.exit(1)
    
    print(f"\nMigrating data to user_id: {DEFAULT_USER_ID}")
    print("\nConfirm? (type 'yes' to proceed): ", end="")
    ...
    with mongo_client() as client:
        ...
        total = 0
        total += migrate_collection(db, "strategies", DEFAULT_USER_ID)
        total += migrate_collection(db, "models.registry", DEFAULT_USER_ID)
        total += migrate_collection(db, "sim_runs", DEFAULT_USER_ID)
        total += migrate_collection(db, "experiments", DEFAULT_USER_ID)
        total += migrate_collection(db, "positions", DEFAULT_USER_ID)
        total += migrate_collection(db, "orders", DEFAULT_USER_ID)
        
        print(f"\n✅ Migrated {total} total documents")
        ...
        print("3. Enable signups (ALLOW_SIGNUPS=true)")
...
export MIGRATION_USER_ID="google_123456789"
...
# Expected output:
# ============================================================
# Phase 2 Migration: Adding user_id to collections
# ============================================================
# 
# Migrating data to user_id: google_123456789
# 
# Confirm? (type 'yes' to proceed): yes
# 
# 📋 Migrating collections...
#   ✅ strategies: Migrated 15 documents
#   ✅ models.registry: Migrated 42 documents
#   ✅ sim_runs: Migrated 128 documents
#   ...
```
...
with mongo_client() as client:
    ...
    # Check strategies
    count = db.strategies.count_documents({"user_id": "google_123456789"})
    print(f"Your strategies: {count}")
    
    # Check models
    count = db["models.registry"].count_documents({"user_id": "google_123456789"})
    print(f"Your models: {count}")
    
    # Verify no orphaned data
    orphaned = db.strategies.count_documents({"user_id": {"$exists": False}})
    print(f"Orphaned strategies: {orphaned}")  # Should be 0
```
